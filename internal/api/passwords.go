...
func (c *Client) ListPasswords() ([]PasswordEntry, error) {
	...
	if err := c.doAuthenticatedRequest(http.MethodGet, c.scopedPath(PathPasswords, nil), nil, &result); err != nil {
		return nil, err
	}
	return result, nil
}
...
func (c *Client) GetPassword(uuid string) (*PasswordEntry, error) {
	path := c.scopedPath(PathPasswords+uuid+"/", nil)
	var result PasswordEntry
...
func (c *Client) CreatePassword(entry PasswordEntry) (*PasswordEntry, error) {
	var result PasswordEntry
	if err := c.doAuthenticatedRequest(http.MethodPost, c.scopedPath(PathPasswords, nil), entry, &result); err != nil {
		return nil, err
	}
	return &result, nil
}
...
func (c *Client) GeneratePassword(opts PasswordGenOpts) (*GeneratedPassword, error) {
	...
	path := c.scopedPath(PathPasswords+"generate-password/", params)
